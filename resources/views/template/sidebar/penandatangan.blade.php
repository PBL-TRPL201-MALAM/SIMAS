<aside id="sidebar" class="fixed inset-y-0 left-0 z-40 flex w-64 -translate-x-full flex-col overflow-y-auto border-r border-slate-100 bg-white shadow-xl transition-all duration-300 xl:relative xl:z-30 xl:h-screen xl:translate-x-0 xl:shadow-none">
  <!-- Sidebar Penandatangan memisahkan pekerjaan persetujuan dan monitoring seluruh dokumen per jenis surat. -->
  ...
  <div class="relative flex items-center gap-2.5 px-5 h-16 border-b border-slate-100/80 shrink-0">
    ...
    <div>
      ...
      <span class="text-[10px] font-medium text-blue-500">Penandatangan</span>
    </div>
  </div>
  ...
  <nav class="flex-1 px-3 py-4 space-y-0.5">
    ...
    <a href="{{ route('penandatangan.dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ $activePage === 'dashboard' ? 'bg-blue-600 text-white shadow-sm shadow-blue-200' : 'text-slate-600 hover:bg-blue-50 hover:text-blue-600' }}">
    ...
    <!-- Badge pada menu Perlu Persetujuan menghitung antrean aktif yang sudah boleh diproses oleh penandatangan login. -->
    <a href="{{ route('penandatangan.surat-menunggu') }}" class="flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ $activePage === 'surat-menunggu' ? 'bg-blue-600 text-white shadow-sm shadow-blue-200' : 'text-slate-600 hover:bg-blue-50 hover:text-blue-600' }}">
    ...
    <a href="{{ route('penandatangan.surat-semua') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ $activePage === 'surat-semua' ? 'bg-blue-600 text-white shadow-sm shadow-blue-200' : 'text-slate-600 hover:bg-blue-50 hover:text-blue-600' }}">
    ...
    <a href="{{ route('penandatangan.sk-menunggu') }}" class="flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ $activePage === 'sk-menunggu' ? 'bg-blue-600 text-white shadow-sm shadow-blue-200' : 'text-slate-600 hover:bg-blue-50 hover:text-blue-600' }}">
      <div class="flex items-center gap-3">
        <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        <span>Perlu Persetujuan SK</span>
      </div>
    ...
    <a href="{{ route('penandatangan.sk-semua') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ $activePage === 'sk-semua' ? 'bg-blue-600 text-white shadow-sm shadow-blue-200' : 'text-slate-600 hover:bg-blue-50 hover:text-blue-600' }}">
  ...
  <div class="px-3 py-4 border-t border-slate-100/80 shrink-0 space-y-0.5">
    <!-- Area bawah sidebar berisi profil penandatangan dan form logout. -->
    <a href="{{ route('penandatangan.profil') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 {{ $activePage === 'profil' ? 'bg-blue-600 text-white shadow-sm shadow-blue-200' : 'text-slate-600 hover:bg-blue-50 hover:text-blue-600' }}">
