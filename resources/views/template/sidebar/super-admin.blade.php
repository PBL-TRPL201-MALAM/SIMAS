<aside id="sidebar" class="fixed inset-y-0 left-0 z-40 flex w-64 -translate-x-full flex-col overflow-y-auto border-r border-slate-100 bg-white shadow-xl transition-all duration-300 xl:relative xl:z-30 xl:h-screen xl:translate-x-0 xl:shadow-none">
  ...
  <!-- Sidebar Super Admin berisi akses pengelolaan user, master data, dan monitoring sistem. -->
  ...
  <nav class="flex-1 px-3 py-4 space-y-0.5">
    ...
    <!-- $sidebarStats['user_count'] dipakai untuk badge jumlah user pada menu Semua User. -->
    @php($sidebarStats = $sidebarStats ?? ['user_count' => 0])

    <!-- request()->routeIs() memberi class aktif berdasarkan nama route yang sedang dibuka. -->
  ...
  <div class="px-3 py-4 border-t border-slate-100/80 shrink-0 space-y-0.5">
    <!-- Area bawah sidebar berisi profil Super Admin dan form logout. -->
    ...
    <form action="{{ route('logout') }}" method="POST" class="w-full">
      <!-- csrf melindungi form logout yang mengubah session autentikasi Laravel. -->
      @csrf
