@include('template.layouts.header', ['pageTitle' => 'Detail Persetujuan Surat'])
@include('template.sidebar.penandatangan', ['activePage' => $activePage ?? 'surat-menunggu'])
    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
      <header class="flex items-center justify-between h-16 px-6 bg-white border-b border-slate-100/80 shrink-0">
        <div>
          <h1 class="text-sm font-bold text-slate-900">Detail Persetujuan Surat</h1>
          <p class="text-[11px] text-slate-400 font-light">{{ ($isReadOnly ?? false) ? 'Baca dokumen dalam mode pemantauan read-only.' : 'Baca dokumen dan berikan persetujuan akhir dari halaman ini.' }}</p>
        </div>
        <a href="{{ $backUrl ?? route('penandatangan.surat-menunggu') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-xs font-medium text-slate-600 hover:border-slate-300 hover:text-slate-800 transition-all duration-200">
          ...
          <div class="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_380px]">
            ...
            <aside class="space-y-4">
                        ...
                        <span class="inline-flex shrink-0 items-center gap-1">
                          @if ($lampiran->isPreviewableLampiran())
                            @if ($canAccessAssignedFiles ?? false)
                              <a href="{{ route('penandatangan.lampiran.preview', $lampiran) }}" target="_blank" rel="noopener" class="rounded-lg bg-blue-50 px-2.5 py-1 text-[10px] font-semibold text-blue-600 hover:bg-blue-100 transition-all duration-200">Lihat</a>
                            @endif
                          @endif
                          @if ($canAccessAssignedFiles ?? false)
                            <a href="{{ route('penandatangan.lampiran.download', $lampiran) }}" class="rounded-lg bg-white px-2.5 py-1 text-[10px] font-semibold text-slate-600 hover:bg-slate-100 transition-all duration-200">Unduh</a>
                          @endif
                        </span>
                      </div>
                    @endforeach
              ...
              <section class="rounded-2xl border border-slate-100 bg-white p-5 space-y-4">
                <div>
                  <h2 class="text-sm font-bold text-slate-900">Aksi Persetujuan Akhir</h2>
                  <p class="text-[11px] text-slate-400 font-light mt-1">Setujui dan tandatangani untuk meneruskan proses, atau tolak dengan catatan yang jelas.</p>
                </div>
                ...
                @if ($canProcess && $verifikasi)
                  ...
                  <form action="{{ route('penandatangan.verifikasi.proses', $verifikasi) }}" method="POST">
                  ...
                  <form action="{{ route('penandatangan.verifikasi.proses', $verifikasi) }}" method="POST" class="space-y-3 rounded-2xl border border-red-100 bg-red-50/60 p-4">
                ...
                @elseif ($isReadOnly ?? false)
                  <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                    ...
                    <p class="mt-1 text-[11px] font-light text-slate-400">Aksi persetujuan hanya tersedia pada menu Perlu Persetujuan.</p>
                  </div>
                @else
                  <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
