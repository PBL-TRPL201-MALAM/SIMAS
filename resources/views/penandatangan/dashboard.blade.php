...
@include('template.sidebar.penandatangan', ['activePage' => 'dashboard'])
    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
      <header class="flex items-center justify-between h-16 px-6 bg-white border-b border-slate-100/80 shrink-0">
        ...
        <div>
          ...
          <p class="text-[11px] text-slate-400 font-light">Selamat datang, Penandatangan</p>
        </div>
      ...
      <main class="flex-1 overflow-y-auto p-6">
        <div id="page-dashboard" class="page-content space-y-6">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h2 class="text-base font-bold text-slate-900">Halo, Penandatangan!</h2>
              <p class="text-xs text-slate-400 font-light mt-0.5">Ada <strong class="text-blue-600">{{ $stats['menunggu'] }} dokumen</strong> yang menunggu persetujuan tanda tangan Anda.</p>
            </div>
            <a href="{{ route('penandatangan.surat-menunggu') }}" class="inline-flex items-center gap-1.5 rounded-xl bg-blue-600 px-3.5 py-2 text-xs font-semibold text-white shadow-sm hover:bg-blue-700 transition-all duration-200">
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="rounded-2xl bg-white border border-slate-100 p-5 hover:shadow-md hover:shadow-blue-50/60 transition-all duration-300">
              ...
              <p class="text-xs text-slate-400 font-light mt-0.5">Menunggu Persetujuan</p>
            </div>
            <div class="rounded-2xl bg-white border border-slate-100 p-5 hover:shadow-md hover:shadow-blue-50/60 transition-all duration-300">
              <div class="w-9 h-9 rounded-xl bg-blue-50 flex items-center justify-center mb-3">
            ...
            <div class="rounded-2xl bg-white border border-slate-100 p-5 hover:shadow-md hover:shadow-blue-50/60 transition-all duration-300">
              ...
              <p class="text-xs text-slate-400 font-light mt-0.5">Total Diproses</p>
            </div>
          </div>

          <!-- Chart + Table: Status Verifikasi + Menunggu -->
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div class="rounded-2xl bg-white border border-slate-100 p-5 hover:shadow-md hover:shadow-blue-50/60 transition-all duration-300">
              <div class="flex items-center justify-between mb-4">
                <div>
                  <h3 class="text-sm font-semibold text-slate-800">Status Persetujuan Saya</h3>
                  <p class="text-[11px] text-slate-400 font-light mt-0.5">Distribusi keputusan persetujuan</p>
                </div>
            ...
            <div class="lg:col-span-2 rounded-2xl bg-white border border-slate-100 overflow-hidden">
              <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                <h3 class="text-sm font-semibold text-slate-800">Menunggu Persetujuan Saya</h3>
                <a href="{{ route('penandatangan.surat-menunggu') }}" class="text-[11px] font-medium text-blue-500 hover:text-blue-700 transition-colors duration-200">Lihat semua</a>
              </div>
                  ...
                  <tbody class="divide-y divide-slate-50">
                    ...
                    @forelse ($latestPending as $item)
                      ...
                      @php
                        ...
                        $detailRoute = $isSk
                            ? route('penandatangan.sk.detail', $item->dokumen)
                            : route('penandatangan.surat.detail', $item->dokumen);
                      @endphp
                      <tr class="hover:bg-slate-50/40 transition-colors duration-150">
                        ...
                        <td class="px-5 py-3.5">
                          ...
                          <a href="{{ $detailRoute }}" class="inline-flex items-center gap-1 text-[11px] font-semibold text-white bg-blue-600 hover:bg-blue-700 px-2.5 py-1 rounded-lg transition-all duration-200">Tinjau</a>
                        </td>
                      </tr>
                    @empty
                      <tr>
                        <td colspan="6" class="px-5 py-10 text-center text-xs text-slate-400">Belum ada dokumen yang menunggu persetujuan Anda.</td>
                      </tr>
                    @endforelse
                  </tbody>
                </table>
...
document.addEventListener('DOMContentLoaded', function () {
    const chartData = @json($chartVerifikasi);
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    data: chartData.data,
                    backgroundColor: [
                        'rgba(59, 130, 246, 0.8)',   // Blue 500 — Menunggu
                        'rgba(16, 185, 129, 0.8)',   // Emerald 500 — Disetujui
                        'rgba(244, 63, 94, 0.8)',    // Rose 500 — Ditolak
                    ],
                    borderColor: [
                        'rgba(59, 130, 246, 1)',
                        'rgba(16, 185, 129, 1)',
