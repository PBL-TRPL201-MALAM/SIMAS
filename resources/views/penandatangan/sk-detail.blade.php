@include('template.layouts.header', ['pageTitle' => 'Detail Persetujuan SK'])
@include('template.sidebar.penandatangan', ['activePage' => $activePage ?? 'sk-menunggu'])
...
@endphp
    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
      <header class="flex items-center justify-between h-16 px-6 bg-white border-b border-slate-100/80 shrink-0">
        ...
        <div>
          <h1 class="text-sm font-bold text-slate-900">Detail Persetujuan SK</h1>
          <p class="text-[11px] text-slate-400 font-light">{{ ($isReadOnly ?? false) ? 'Tinjau isi SK dalam mode pemantauan read-only.' : 'Tinjau isi SK dan berikan persetujuan akhir sesuai kewenangan Anda.' }}</p>
        </div>
        <a href="{{ $backUrl ?? route('penandatangan.sk-menunggu') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-xs font-medium text-slate-600 hover:border-slate-300 hover:text-slate-800 transition-all duration-200">
            ...
            <aside class="space-y-4">
              ...
              <section class="rounded-2xl border border-slate-100 bg-white p-5 space-y-4">
                <div>
                  ...
                  <p class="text-[11px] text-slate-400 font-light mt-1">Setujui dan tandatangani jika isi SK sudah benar, atau kembalikan revisi dengan catatan untuk Pemohon.</p>
                </div>
                ...
                @if ($canProcess && $verifikasi)
                  ...
                  <form action="{{ route('penandatangan.verifikasi.proses', $verifikasi) }}" method="POST" class="space-y-3 rounded-2xl border border-amber-100 bg-amber-50/60 p-4">
                    @csrf
