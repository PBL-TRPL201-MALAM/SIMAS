@include('template.layouts.header', ['pageTitle' => 'Perlu Verifikasi Surat'])
@include('template.sidebar.verifikator', ['activePage' => 'surat-menunggu'])
@php
...
@endphp
    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
      <header class="flex items-center justify-between h-16 px-6 bg-white border-b border-slate-100/80 shrink-0">
        ...
        <div>
          <h1 class="text-sm font-bold text-slate-900">Perlu Verifikasi Surat</h1>
          <p class="text-[11px] text-slate-400 font-light">Daftar surat yang menjadi tugas verifikasi Anda.</p>
        </div>
        ...
        <div id="page-surat-menunggu" class="page-content space-y-4">
          ...
          <div class="inline-flex flex-wrap items-center gap-1 rounded-xl border border-slate-200 bg-white p-1">
            @foreach ($statusTabs as $statusKey => $statusLabel)
              ...
              @endphp
              <a href="{{ route('verifikator.surat-menunggu', $statusKey === 'menunggu' ? [] : ['status' => $statusKey]) }}"
                    ...
                    <tr class="hover:bg-slate-50/40 transition-colors duration-150">
                      ...
                      <td class="px-5 py-3.5">
                        <div class="flex flex-wrap items-center gap-2">
                          <a href="{{ route('verifikator.surat.detail', $item->dokumen) }}" class="inline-flex items-center text-[11px] font-semibold text-white bg-blue-600 hover:bg-blue-700 px-2.5 py-1 rounded-lg transition-all duration-200">Detail</a>
                        </div>
                      </td>
                    </tr>
                  @empty
